...
///Sous-programme pour les connexités
void Graph::connexite()
{
    ///Déclarations des variables
    std::vector<Vertex> Copy ; //on crée un vecteur copie dans le lequel on va met les sommets adjacents appartenant à la composante connexe
    std::stack<int> pile; //on créer notre pile qui symbolise les composantes connexes
    int m = 0, i = 0, a;
    std::vector<bool> SommetMarques; //booléen permettant de savoir si on a deja tester un sommet
    ...
    for(auto& elem : m_vertices) //boucle parcourant la liste des sommets
        Copy.push_back(elem.second); //on ajoute les indices des sommets au vecteur appelé copie
    ...
    SommetMarques[m] = true; //On marque un sommet donc le boléan prd la valeur vraie
    pile.push(m); //on ajoute ce sommet à la pile (composante connexe)
    ...
    while(i < Copy.size()) //tant qu'on a pas fini de parcourir le vecteur copie
    {
        ...
        {
            m=pile.top(); //le premier sommet marqué se trouve en haut de la pile
            pile.pop(); //on ajoute des sommets dans la pile

            for (a=0; a<SommetAdjacent[m].size(); ++a)
            {
                if (!SommetMarques.at(SommetAdjacent[m][a])) //si le sommet marqué n'est pas adjacent à un certain sommet
                {
                    SommetMarques[SommetAdjacent[m][a]] = true;
                    pile.push(SommetAdjacent[m][a]); //on ajoute celui si à la pile donc à la composante connexe
                }
            }
            std::cout << m << std::endl; //on affiche l'indice des sommets de la composante connexe
        }

        i = 0; //on réinitialise i à zero
        while(i < Copy.size() && SommetMarques[i]) //tant que i est inférieur à la taille du vecteur copie et au nb de sommest marqués
            i++; //i est incrémenté

        if(SommetMarques[i]) //si on retombe sur un sommet sur lequel on est deja passé
            break; //alors la composante est finie

        else
        {
            SommetMarques[i] = true;
            pile.push(i); //sinon celui-ci est ajouté à la composante
        }
    }
}

///Sous-programme d'évolution dynamique
