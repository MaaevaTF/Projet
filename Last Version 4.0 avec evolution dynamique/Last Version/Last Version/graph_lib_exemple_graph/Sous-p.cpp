...
void menu (BITMAP* ecran, BITMAP* Menu,BITMAP* Informations, BITMAP* Reseaux, BITMAP* Quitter, int fin)
{
    ...
    while (!fin && v==0) //tant qu'on est dans la boucle intérative
    {
        ...
        if (mouse_x>=309 && mouse_x<=710 && mouse_y>=144 && mouse_y<=262) //si la souris se trouvent dans cette zone
        {
            blit(Informations,ecran,0,0,0,0,1024,768); //on affiche la bitmap des infos
            if(mouse_b&1)
            {
                v = 1 ;
            }
        ...
        if (mouse_x>=309 && mouse_x<=710 && mouse_y>=320 && mouse_y<=441)
        {   clear_bitmap(ecran);
            blit(Reseaux,ecran,0,0,0,0,1024,768); //on affiche la bitmap des réseaux
            if (mouse_b&1) //si on clique sur le bouton gauche
            {
                ...
                blit(Info,ecran,0,0,0,0,1024,768); //Affichage du bitmap explicatif de nos réseaux
                v=2;
            }
        }

        if (mouse_x>=242 && mouse_x<=559 && mouse_y>=493 && mouse_y<=616)
        {   clear_bitmap(ecran);
            ...
            if(mouse_b&1)
            {
                ...
                blit(Bye,ecran,0,0,0,0,1024,768); //Affichage du bitmap explicatif de nos réseaux
                v=3;
            }
        }
...
void choix (BITMAP* ecran, BITMAP* Choice, BITMAP* Eau,BITMAP* Foret, BITMAP* Savane,BITMAP* Menu,BITMAP* Informations, BITMAP* Reseaux, BITMAP* Quitter) //sous-p du choix du réseau à afficher
{
    ...
    while (fin) //tant qu'on est dans la boucle
    {
        ...
        blit(Choice,ecran,0,0,0,0,1024,768); //on affiche le bitmap de choix du réseau
        rest(100);
        if (mouse_x>=309 && mouse_x<=710 && mouse_y>=144 && mouse_y<=262) //en fonction de la position de la souris
        {   clear_bitmap(ecran);
            blit(Eau,ecran,0,0,0,0,1024,768); //on affiche le bitmap pour le cycle de l'eau
            if (mouse_b&1)
            {
        ...
        if (mouse_x>=309 && mouse_x<=710&& mouse_y>=320 && mouse_y<=441)
        {   clear_bitmap(ecran);
            blit(Savane,ecran,0,0,0,0,1024,768);  //on affiche le bitmap pour le réseau de la savane
            if (mouse_b&1)
            {
                 v = 1 ;
               cond = 2 ;
        ...
        if (mouse_x>=309 && mouse_x<=710&& mouse_y>=493 && mouse_y<=616)
        {   clear_bitmap(ecran);
            blit(Foret,ecran,0,0,0,0,1024,768);  //on affiche le bitmap pour les chaines alimentaires
            if (mouse_b&1)
            {
                 v = 1 ;
               cond = 3;
...
void sauvegarde(Graph g,std::string name)
{
